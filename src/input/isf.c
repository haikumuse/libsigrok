...
/*
 * Both glibc (Linux) <endian.h> and Darwin <sys/_endian.h> define BYTE_ORDER
 * as a macro (glibc: #define BYTE_ORDER __BYTE_ORDER; Darwin: expands to
 * __DARWIN_LITTLE_ENDIAN = 1234). This conflicts with the enum value
 * BYTE_ORDER = 5 below, causing "expected identifier before numeric constant"
 * errors. Undefine the system macro so the enum identifier takes precedence.
